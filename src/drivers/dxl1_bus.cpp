...
Dxl1Bus dxl1Bus(HW::SERVO_DE_PIN);
...
bool Dxl1Bus::eepromWriteByte(uint8_t id, uint8_t addr, uint8_t val) {
  ...
  delay(20); // Flash settle — MX-28 can take >5ms; match DXL2's 20ms for safety
  return true;
}

bool Dxl1Bus::eepromWriteWord(uint8_t id, uint8_t addr, uint16_t val) {
  if (!writeWord(id, addr, val)) return false;
  delay(20); // Flash settle — match DXL2's 20ms for back-to-back write safety
  return true;
}
