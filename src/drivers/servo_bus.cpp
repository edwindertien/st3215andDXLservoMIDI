...
void ST3215Bus::begin(HardwareSerial& serial, uint32_t baud) {
  ...
  _servo.IOTimeOut = 5;   // 5ms — ST3215 responds in <1ms; 20ms blocked the app loop
  _servo._baud     = baud; // EchoSMS_STS uses this for the drain window timing
  _serial->end();
  _serial->begin(baud);
  delay(5);
}
...
void ST3215Bus::setBaud(uint32_t baud) {
  if (!_serial) return;
  _baud        = baud;
  _servo._baud = baud; // keep drain window timing correct after baud change
  _serial->end();
  _serial->begin(baud);
  delay(5);
}
